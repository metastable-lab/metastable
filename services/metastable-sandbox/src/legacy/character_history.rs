...
use metastable_runtime::{CharacterFeature, ToolCall};
use metastable_runtime::{BackgroundStories, BehaviorTraits, Relationships, SkillsAndInterests};
...
impl CharacterHistory {
    pub fn into_new_character_history(&self) -> NewCharacterHistory {
        NewCharacterHistory {
            id: self.id,
...
fn parse_prefixed_string<T>(
    ...
    for s in legacy_values {
        ...
        for (prefix, constructor) in prefix_map {
            if let Some(content) = s.strip_prefix(prefix) {
